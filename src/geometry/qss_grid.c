...
Grid *createGridSetDx(
      ...
      QSSLIB_REAL *x_lo,
      QSSLIB_REAL *x_hi,
      QSSLIB_SPATIAL_DERIVATIVE_ACCURACY_TYPE accuracy)
{
  int     i;
  QSSLIB_REAL  diff;
...
void writeGridToBinaryFile(Grid *grid, char *file_name, int zip_status)
{
  ...
  fwrite(grid->x_lo, sizeof(QSSLIB_REAL), 3, fp);
  fwrite(grid->x_hi, sizeof(QSSLIB_REAL), 3, fp);
  fwrite(grid->x_lo_ghostbox, sizeof(QSSLIB_REAL), 3, fp);
  fwrite(grid->x_hi_ghostbox, sizeof(QSSLIB_REAL), 3, fp);
  fwrite(grid->grid_dims, sizeof(int), 3, fp); 
  fwrite(grid->grid_dims_ghostbox, sizeof(int), 3, fp); 
  fwrite(grid->dx, sizeof(QSSLIB_REAL), 3, fp);
  ...
  fwrite(&(grid->beta),   sizeof(QSSLIB_REAL), 1, fp);
  fwrite(&(grid->gamma),  sizeof(QSSLIB_REAL), 1, fp);
...
Grid *readGridFromBinaryFile(char *file_name)
{
 ...
 /* open file and allocate Grid */
  ...
  if( fp != NULL)
  {
    ...
    fread(grid->x_lo, sizeof(QSSLIB_REAL), 3, fp);
    fread(grid->x_hi, sizeof(QSSLIB_REAL), 3, fp);
    fread(grid->x_lo_ghostbox, sizeof(QSSLIB_REAL), 3, fp);
    fread(grid->x_hi_ghostbox, sizeof(QSSLIB_REAL), 3, fp);
    
    fread(grid->grid_dims, sizeof(int), 3, fp); 
    fread(grid->grid_dims_ghostbox, sizeof(int), 3, fp);
     
    fread(grid->dx, sizeof(QSSLIB_REAL), 3, fp);
    ...
    fread(&(grid->beta),   sizeof(QSSLIB_REAL), 1, fp);
    fread(&(grid->gamma),  sizeof(QSSLIB_REAL), 1, fp);
...
void setIndexSpaceLimits(QSSLIB_SPATIAL_DERIVATIVE_ACCURACY_TYPE accuracy, 
   Grid *grid)
{  
   grid->mark_gb = 120;
